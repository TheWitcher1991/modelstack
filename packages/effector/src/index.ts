...
import type { Dictionary, EntityDefinition, EntityQuery, EntityRead } from '@modelstack/core'
...
export function createEntityListStore<TEntity extends EntityDefinition<string, string, any>>(
	_entity: TEntity,
	...
	const $store = createStore(initialState)
		...
		.on(setFilter, (state, filter) => ({ ...state, filter: { ...state.filter, ...filter } }))
